...
class GapBuffer {
    private:
        ...
        template <typename T>
        void accept(size_t from, size_t to, void (T::*write)(utils::Slice<uint8_t>), T& v) {
            auto start = _bytes.get() + from;
            v.write(utils::Slice<uint8_t> { start, to - from + 1 });
        }
    ...
    public:
        ...
        template <typename T>
        void accept(void (T::*write)(utils::Slice<uint8_t>), T& v) {
