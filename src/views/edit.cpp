...
void views::edit(const EditorState& state, const settings::Config& config, buffers::Buffer& screen_buffer) {
    ...
    const auto text = slice::sub(full_text, state.visible_region);
    //const auto start_pos = views::syntax(text, config.grammar, screen_buffer);
    const auto start_pos = views::plain_text(text, screen_buffer);
 
    const auto cursor = navigation::text_cursor(full_text, state.pos);

    const auto line_start = slice::count(text, ascii::Lf, 0, state.visible_region.start);
    const auto line_end = slice::count(text, ascii::Lf) + line_start;
    const auto line_range = Range<size_t> {
        start: line_start,
        end: line_end
    };
